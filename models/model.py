...
class ArgsRec(nn.Module):
    def __init__(self, config, hidden_size, num_labels, seq_len, pos_emb_size):
        ...
        self.register_buffer('role_indices', torch.arange(0, num_labels, 1).long())
        self.role_emb = nn.Embedding(num_labels, config.hidden_size)
        self.Predictor = AdaptiveAdditionPredictor(config.hidden_size, dropout_rate=config.decoder_dropout)
    ...
    def forward(self, text_emb, relative_pos, trigger_mask, mask, type_emb):
        ...
        role_emb = self.role_emb(self.role_indices) # b, l
        pred_role = self.Predictor(role_emb, h_sa, mask)  # [b, r]
        type_soft_constrain = torch.sigmoid(pred_role)

        # type_soft_constrain = torch.sigmoid(self.gate_linear(type_emb))  # [b, l]
        type_soft_constrain_ = type_soft_constrain.unsqueeze(1).expand_as(p_s)
        p_s = p_s * type_soft_constrain_
        p_e = p_e * type_soft_constrain_
...
class CasEE(nn.Module):
    ...
    def forward(self, tokens, segment, mask, type_id, type_vec, trigger_s_vec, trigger_e_vec, relative_pos, trigger_mask, args_s_vec, args_e_vec, args_mask):
        ...
        role_loss = self.loss_0(type_soft_constrain, args_mask)
        ...
        loss = type_loss + trigger_loss + args_loss + role_loss
